...
- 免费版 JSON 导出是用户备份格式，应包含书签、标签、文件夹和关联关系，但仅限最近 200 条可管理书签。
...
付费墙触发点：
- 导入后本地书签超过 200 条，只展示和管理最近 200 条，提示升级。
- 用户点击 Markdown/CSV 导出。
- 用户点击批量操作。
- 用户进入高级筛选。

免费/Pro 数据规则：
- 免费用户导入超过 200 条后，超出部分仍保存在 IndexedDB，不在默认列表中展示。
- 免费用户的计数口径是未删除书签按 `importedAt` 倒序的最近 200 条。
- 免费用户不能对锁定书签做搜索、标签、文件夹、归档、删除、批量操作或导出。
- 升级 Pro 后，已导入但被锁定的书签立即变为可见和可管理，不需要重新导入。
...
- 新建、重命名、删除文件夹正常。
- 添加、移除标签正常。
- 免费用户超过 200 条后能看到升级提示。
- 免费用户只能搜索、整理和导出最近 200 条未删除书签。
- Pro 激活后历史已导入书签全部恢复可见。
- Pro 激活后解除数量和导出限制。
